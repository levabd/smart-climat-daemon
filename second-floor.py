...
#     """Turn on humidifier on a second floor."""
#     cP = chuangmi_plug.ChuangmiPlug(ip='192.168.19.61', token='eca25f7d91a6034a978af9900ff2d3f2', start_id=0, debug=1, lazy_discover=True, model='chuangmi.plug.m1')
#     cP.on()


# def turn_off_humidifier():
#     """Turn off humidifier on a second floor."""
#     cP = chuangmi_plug.ChuangmiPlug(ip='192.168.19.61', token='eca25f7d91a6034a978af9900ff2d3f2', start_id=0, debug=1, lazy_discover=True, model='chuangmi.plug.m1')
#     cP.off()
...
def recordTempHumid(temperature, humidity, room):
    dicty = {
...
def main():
    ...
    # cP = chuangmi_plug.ChuangmiPlug(ip='192.168.19.61', token='14f5b868a58ef4ffaef6fece61c65b16', start_id=0, debug=0, lazy_discover=True, model='chuangmi.plug.m1')
    # print(cP.status())

    # Record temperature and humidity for monitor
    recordTempHumid(temperature, humidity, 'br')
    ...
    with open('/home/pi/smart-climat-daemon/ac_br_state.json', 'w') as f:
        json.dump(state, f)

    if (temperature > 24.0) and (today.month < 6) and (today.month > 3) and (today.hour < 11) and (today.hour > 3):
        turn_on_cool_ac()
    if (temperature > 27) and (today.hour < 24) and (today.hour > 10):
        turn_on_cool_ac()
    if (temperature > 24.9) and (today.month < 10) and (today.month > 5) and (today.hour < 11) and (today.hour > 3):
        turn_on_cool_ac()
    if (temperature < 22.5):
        turn_off_ac()
    ...
    (today, temperature, humidity) = poll_temp_humidity('cb')
    recordTempHumid(temperature, humidity, 'cb')
